...
class ${classInfo.className}Resource(
    ...
    fun insert(ctx: Context) {
        ...
        articleService.insert(${classInfo.className?uncap_first}, ctx.reqLog(ctx.userContext().uid))
        ctx.status(204)
    }
    ...
    fun delete(ctx: Context) {
        ...
        ${classInfo.className?uncap_first}Service.delete(id, ctx.reqLog(ctx.userContext().uid))
        ctx.status(204)
    }
    ...
    fun update(ctx: Context) {
        val ${classInfo.className?uncap_first} = ctx.bodyAsClass(${classInfo.className}::class.java)
        ${classInfo.className?uncap_first}Service.update(${classInfo.className?uncap_first}, ctx.reqLog(ctx.userContext().uid))
        ctx.status(204)
    }
    ...
    fun findById(ctx: Context) {
        val id = ctx.pathInt("id")
        val ${classInfo.className?uncap_first} = ${classInfo.className?uncap_first}Service.findById(id) ?: throw BizCodeException(BizCodes.C_10001)
